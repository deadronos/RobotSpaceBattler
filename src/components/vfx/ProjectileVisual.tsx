...
function RocketProjectileVisual({
  projectile,
}: {
  projectile: ProjectileEntity;
}) {
    ...
    >
      <group ref={groupRef}>
        <mesh rotation={[Math.PI / 2, 0, 0]}>
          <cylinderGeometry
            args={[radius * 0.35, radius * 0.45, radius * 2, 12]}
          />
...
function GunProjectileVisual({ projectile }: { projectile: ProjectileEntity }) {
  ...
  const length =
    direction.length() > 0
      ? Math.min(1.4, Math.max(0.35, direction.length() * 0.04))
      : 0.45;

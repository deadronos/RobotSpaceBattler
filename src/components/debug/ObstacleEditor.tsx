...
const styles: Record<string, React.CSSProperties> = {
  ...
  button: {
    ...
    border: "none",
    cursor: "pointer",
    background: "linear-gradient(135deg, #7ae0ff, #5f7bff)",
    color: "#0b1024",
    fontWeight: 700,
    fontSize: 12,
  },
...
export function ObstacleEditor({ world, sampleFixture }: ObstacleEditorProps) {
  ...
  const [fixtureText, setFixtureText] = useState(() =>
    serializeObstacleFixture(exportObstacleFixture(world)),
  );
      ...
      <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
        ...
        {sampleFixture ? (
          <button
            ...
            style={{
              ...styles.button,
              background: "linear-gradient(135deg, #9cffc7, #5bcc9c)",
            }}
            onClick={loadSample}
          >
            Load Sample
        ...
        <button
          ...
          style={{
            ...styles.button,
            background: "linear-gradient(135deg, #ffc97a, #ff8f5f)",
          }}
          onClick={copyFixture}
        >
          Copy JSON

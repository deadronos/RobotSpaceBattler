...
type Entity = {
  ...
  weapon?: { cooldown: number; fireRate: number; range: number; damage: number; projectileSpeed: number; projectileTTL: number }
  projectile?: { ttl: number; damage: number; radius: number; velocity: { x: number; y: number; z: number } }
  fx?: { muzzleTimer?: number }
}

function createRobotEntity(id: string, team: 'red' | 'blue', pos: THREE.Vector3) {
  store.add({
    id,
    team,
    position: pos.toArray(),
    health: { hp: 10, maxHp: 10 },
    weapon: { cooldown: 0, fireRate: 2, range: 12, damage: 2, projectileSpeed: 25, projectileTTL: 3 }, // fireRate shots/sec
    fx: {}
  })
}
...
export default function Simulation() {
  ...
  useFrame((_, delta) => {
    ...
    for (const p of [...store.entities.values()]) {
      ...
      const pos = p.rb ? p.rb.translation() : { x: p.position[0], y: p.position[1], z: p.position[2] }
    ...
    const redAlive = [...store.entities.values()].filter(e => e.team === 'red' && e.health).length
    const blueAlive = [...store.entities.values()].filter(e => e.team === 'blue' && e.health).length
  ...
  if (!e.rb) return
      ...
      for (const enemy of enemies) {
        ...
        if (d2 < bestDist) { bestDist = d2; best = enemy as Entity }
      }
      ...
      if (e.weapon) {
        ...
        if (e.weapon.cooldown <= 0 && dist2 <= range2) {
          ...
          spawnProjectile(e as Entity, new THREE.Vector3(from.x, from.y + 1, from.z), fireDir, e.weapon.damage)
  ...
  return (
    <>
      ...
      {[...store.entities.values()].filter((e) => !e.projectile).map((e) => (
        <Robot
          key={e.id}
          team={e.team}
      ...
      {[...store.entities.values()].filter((e) => !!e.projectile).map((p) => (
        <Projectile
          key={p.id}
          id={p.id}
          team={p.team}

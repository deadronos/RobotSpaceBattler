...
export function updateHazardSystem(world: BattleWorld, deltaMs: number, telemetry?: TelemetryPort): void {
  ...
  for (const robot of world.robots.entities) {
    ...
    robot.statusExpirations = robot.statusExpirations.filter((entry) => entry.expiresAt > now);
    if (robot.statusFlags) {
      robot.statusFlags = robot.statusFlags.filter((f) =>
        robot.statusExpirations!.some((e) => e.flag === f) || (robot.statusFlags && robot.statusFlags.includes(f)),
      );
          ...
          for (const effect of obstacle.hazardEffects) {
            if (effect.kind === 'damage' && effect.perSecond) {
              ...
              if (effect.durationMs && effect.durationMs > 0) {
                ...
                const existing = robot.statusExpirations.find((e) => e.flag === flag);
                if (existing) {
                ...
                if (!robot.statusFlags.includes(flag)) robot.statusFlags.push(flag);
              } else {
  ...
  for (const robot of world.robots.entities) {
    ...
    const validExpirations = new Set((robot.statusExpirations ?? []).map((e) => e.flag));

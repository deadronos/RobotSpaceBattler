...
function normalizeMovementPattern(
  pattern: ObstacleFixtureEntry["movementPattern"],
): ObstacleEntity["movementPattern"] {
  ...
  const legacyPath =
    (pattern as { path?: Vec3Input[]; segments?: Vec3Input[] })?.path ??
    (pattern as { path?: Vec3Input[]; segments?: Vec3Input[] })?.segments;
  const hasTarget =
    (pattern as { targetPosition?: Vec3Input; to?: Vec3Input; end?: Vec3Input })
      .targetPosition ??
    (pattern as { targetPosition?: Vec3Input; to?: Vec3Input; end?: Vec3Input })
      .to ??
    (pattern as { targetPosition?: Vec3Input; to?: Vec3Input; end?: Vec3Input })
...
function normalizeObstacle(
  entry: ObstacleFixtureEntry,
  index: number,
): ObstacleEntity {
  const obstacleType =
    (entry.obstacleType as ObstacleEntity["obstacleType"]) ?? "barrier";
  ...
  return {
    ...
    hazardSchedule:
      (entry as { schedule?: ObstacleEntity["hazardSchedule"] }).schedule ??
      entry.hazardSchedule ??
      null,
    hazardEffects:
      (entry as { effects?: ObstacleEntity["hazardEffects"] }).effects ??
      entry.hazardEffects ??
      null,
...
export function spawnObstaclesFromFixture(
  world: BattleWorld,

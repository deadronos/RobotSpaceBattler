...
function selectTarget(world: WorldView, robot: Robot): Robot | null {
  const enemies = getAliveRobots(world).filter(
    (entity) => entity.team !== robot.team,
  );
  if (enemies.length === 0) {
    return null;
  }
...
function chooseBehavior(world: WorldView, robot: Robot): void {
  ...
  const teamRatio = world.teams[opposing]
    ? world.teams[robot.team].activeRobots /
      Math.max(1, world.teams[opposing].activeRobots)
    : 1;
  ...
  if (robot.health <= RETREAT_HEALTH_THRESHOLD) {
    ...
    healthRatio < DEFENSIVE_HEALTH_THRESHOLD ||
    teamRatio < TEAM_DISADVANTAGE_THRESHOLD
  ) {
...
function moveToward(
  ...
  const nextPosition = clampToArena(
    world.arena,
    addVectors(robot.position, step),
  );
...
export function applyIndividualMovement(
  world: WorldView,
  deltaTime: number,
): void {
  getAliveRobots(world).forEach((robot) => {
    ...
    if (robot.aiState.targetId) {
      const target = world.entities.find(
        (entity) => entity.id === robot.aiState.targetId,
      );
      if (target) {

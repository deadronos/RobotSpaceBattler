...
export function applyDamage(
  ...
  setTeamEntity(
    world,
    target.team,
    recordDamageTaken(world.teams[target.team], amount),
  );
  ...
  if (attackerId) {
    ...
    if (attacker) {
      ...
      setTeamEntity(
        world,
        attacker.team,
        recordDamageDealt(world.teams[attacker.team], amount),
      );
      if (target.health === 0) {
        ...
        setTeamEntity(
          world,
          attacker.team,
          recordKill(world.teams[attacker.team]),
        );
      }
    }
  }
...
export function handleProjectileHits(
  world: WorldView,
  hits: PhysicsStepResult["hits"],
): void {
  hits.forEach((hit) => {
    applyDamage(world, hit.targetId, hit.damage, hit.ownerId);
    removeProjectileBody(world.physics, hit.projectileId);
    const index = world.projectiles.findIndex(
      (projectile) => projectile.id === hit.projectileId,
    );
    if (index !== -1) {
...
export function cleanupProjectiles(
  ...
  for (let index = world.projectiles.length - 1; index >= 0; index -= 1) {
    ...
    const remove =
      removalSet.has(projectile.id) || shouldDespawn(projectile, currentTime);
    if (remove) {

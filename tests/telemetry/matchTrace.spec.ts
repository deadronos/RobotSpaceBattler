...
describe("MatchTrace", () => {
  ...
  describe("concurrent matches", () => {
    it("should support multiple concurrent MatchTrace instances", () => {
      ...
      expect(fs.existsSync(path.join(TEST_TRACE_DIR, "match-001.ndjson"))).toBe(
        true,
      );
      expect(fs.existsSync(path.join(TEST_TRACE_DIR, "match-002.ndjson"))).toBe(
        true,
      );
      expect(fs.existsSync(path.join(TEST_TRACE_DIR, "match-003.ndjson"))).toBe(
        true,
      );

      // Verify content
      const content1 = fs.readFileSync(
        path.join(TEST_TRACE_DIR, "match-001.ndjson"),
        "utf-8",
      );
      const content2 = fs.readFileSync(
        path.join(TEST_TRACE_DIR, "match-002.ndjson"),
        "utf-8",
      );
      const content3 = fs.readFileSync(
        path.join(TEST_TRACE_DIR, "match-003.ndjson"),
        "utf-8",
      );
    ...
    it("should isolate events between concurrent matches", () => {
      ...
      const contentA = fs.readFileSync(
        path.join(TEST_TRACE_DIR, "match-A.ndjson"),
        "utf-8",
      );
      const contentB = fs.readFileSync(
        path.join(TEST_TRACE_DIR, "match-B.ndjson"),
        "utf-8",
      );
